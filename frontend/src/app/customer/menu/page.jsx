            ...
            <VoiceLayer>
                <div className="min-h-screen font-[telegraf] p-4 md:p-8" 
                    ...
                    <div className="w-full h-[80vh] flex gap-6 mt-8">
                        {/* Left Pane */}
                        <div
                            className={`w-2/5 ${loading ? "hidden" : "flex"} flex-col h-full items-center relative rounded-[30px] 
                                        transform transition-all duration-700 ease-out ${
                                            animateLeftPane ? "translate-x-0 opacity-100" : "-translate-x-full opacity-0"
                                        }`}
                            style={{
                        ...
                        >
                            <h2 className="text-[40px] font-extrabold mt-6 mb-6 text-center"
                        ...
                        {/* Right Pane */}
                        <div className={`w-3/5 h-full ${loading ? "hidden" : "flex"} flex-col overflow-y-auto`}>
                                ...
                                {categories.map((category, index) => (
                                    <VoiceElement
                                        ...
                                        className={`relative group block transform transition-all duration-500 ease-in-out ${
                                            showRightItems[index] ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
                                        }`}
                                    >
                                        <div
                                        ...
                                        >
                                            ...
                                            {categoryDrinks[category]?.length > 0 && (
                                                <div className="flex justify-between gap-2 px-10 opacity-0 group-hover:h-[15vh] h-[0px] group-hover:opacity-100 transition-all duration-300">
                                                    {categoryDrinks[category].slice(0, 6).map((drink, i) => (
                                                        <img
                                                            key={i}
