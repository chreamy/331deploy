...
function DrinkDetails() {
    ...
    useEffect(() => {
        fetch(`${SERVER}/modifications`)
            ...
            .then((data) => {
                const fetchedModifications = data;
                console.log(fetchedModifications.modifications);
                setModifications(fetchedModifications.modifications); // Update the state
                setLoading(false);
            })
            .catch((err) => {
        ...
        fetch(`${SERVER}/toppings`)
            ...
            .then((data) => {
                console.log(data);
                setToppings(data.toppings); // Update the state
                setLoading(false);
            })
            .catch((err) => {
                console.error("Failed to fetch modifications:", err);
            ...
            <div className="flex flex-col md:flex-row gap-6 flex-grow">
                ...
                <div className="w-full md:w-2/3 flex-grow">
                    <div className="space-y-6">
                                    ...
                                    .map((mod, idx) => (
                                        ...
                                        >
                                            <input
                                                ...
                                                onChange={() =>
                                                    setSelectedIce(mod.name)
                                                }
                                                className="w-5 h-5"
                                            />
                                            <span className="text-[#EED9C4] text-lg">
                                                {mod.name}
                                            </span>
                                        </div>
                                    ))}
                        ...
                        <div className="grid w-[500px] grid-cols-4 mb-4 gap-0">
                            {toppings.map((mod, idx) => (
                                <div
                                    key={idx}
                                    className="flex items-center gap-2"
                                >
